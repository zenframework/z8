...
public class JDXExceptionBreakpoint extends JDXBreakpoint {
    ...
    public JDXExceptionBreakpoint(final IResource resource, final String exceptionName, final boolean caught,
            final boolean uncaught, final boolean checked, final boolean add, final Map<String, Object> attributes)
    ...
    protected EventRequest[] newRequests(JDXDebugTarget target, ReferenceType type) throws CoreException {
        ...
        if(manager == null) {
            target.requestFailed(
                    JDXMessages.JDXExceptionBreakpoint_Unable_to_create_breakpoint_request___VM_disconnected__1, null);
        }
        try {
    ...
    public boolean handleBreakpointEvent(Event event, JDXDebugTarget target, JDXThread thread) {
        if(event instanceof ExceptionEvent) {
            ...
            if(getExclusionClassFilters().length > 1 || getInclusionClassFilters().length > 1
                    || (getExclusionClassFilters().length + getInclusionClassFilters().length) >= 2
                    || filtersIncludeDefaultPackage(m_inclusionClassFilters)
