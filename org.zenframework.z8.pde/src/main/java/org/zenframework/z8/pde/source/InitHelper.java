...
public class InitHelper implements Cloneable {
    ...
    /*public Init2 findInitDeep(){
    	if (context.isEnum()) return new Init2(context,(ILanguageElement)context.findMember(what.get(0)));
    	boolean statical = context.findMember(what.get(0))==null;
    	IType curr = context;
    	while (curr!=null){
    		IInitializer init = curr.getInitializer(toQname(what));
    		if (init!=null){
    			if (init instanceof MemberNestedType) return new Init2(curr, (MemberNestedType)init);
    			ILanguageElement el = init.getRightElement();
    			if (el!=null){
    				InitHelper newHelper = getNewHelper(curr, el);
    				if (newHelper==null) return new Init2(curr, el);
    				else return newHelper.findInitDeep();
    			}
    		}
    		for (int size=what.size()-1; size>0; size--){
    			if (statical && size==1){
    				if (curr.getBaseType()==null){
    					IType sType = context.getCompilationUnit().resolveType(what.get(0));
    					return new InitHelper(sType, what.subList(1, what.size())).findInitDeep();
    				}
    			}
    			else {
    				List<String> list = what.subList(0, size);
    				List<String> post = what.subList(size, what.size());
    				Init2 init2 = new InitHelper(curr, list).findInitDeep();
    				if (init2!=null){
    					ILanguageElement right = init2.element;
    					if (right!=null){
    						if (right instanceof MemberNestedType) {
    							MemberNestedType mnt = (MemberNestedType) right;
    							return new InitHelper(mnt, post).findInitDeep();
    						}
    						if (right instanceof OperatorNew) {
    							OperatorNew oNew = (OperatorNew) right;
    							return new InitHelper(oNew.getVariableType().getType(), post).findInitDeep();
    						}
    						InitHelper newHelper = getNewHelper(context, right);
    						if (newHelper==null) return null;
    						newHelper.what.addAll(post);
    						return newHelper.findInitDeep();
    					}
    				}
    			}
    		}
    		
    		if (curr.getMember(what.get(0))!=null) break;
    ...
    public Init2 findInitDeep() {
        ...
        if (init2==null) return null;
        InitHelper helper = createHelper(init2);
        if (helper==null) return init2;
        if (helper.what.size()==0) return new Init2(init2.context, (ILanguageElement) helper.context);
        if (this.equals(helper))
        	return new Init2(context, what.subList(0, what.size()-1), getMember().getVariableType().getType(), null);
        findInitDeepHelpers.add(helper);
        Init2 result = helper.findInitDeep();
        findInitDeepHelpers.addAll(helper.findInitDeepHelpers);
        return result;*/
                ...
                if(arrayType.isAuto()) {
                    ...
                    for(int i = members.length - 1; i >= 0; i--) {
                        ...
                        if(!memberType.isArray())
                            if(memberType.getType().equals(arrayType.getType())
                                    || memberType.getType().isSubtypeOf(arrayType.getType()))
                        ...
                        if(right != null) {
                            ...
                            if(newHelper != null) {
                                ...
                                if(!newHelper.equals(this)) {
                                    ...
                                    if(right instanceof MemberNestedType || right instanceof OperatorNew
                                            || right instanceof IType) {
                                        for(Init2 i : addition) {
                                            //int contextUp=0;
                                            if(i.context == newHelper.context) {
                                                /*	IType t = newHelper.context;
                                                	while (t!=null && t!=i.context){
                                                		t=t.getContainerType();
                                                		contextUp++;
                                                	}
                                                	if (t==null) contextUp=1/0; // this shouldn't have happened!
                                                }
                                                if (contextUp==0){*/
                                                i.context = context;
    ...
    /*public IMember getMember(){
    	IType cont = context;
    	if (what.size()==0) return null;
    	for (int i=0; i<what.size()-1; i++){
    		String qName = what.get(i);
    		IMember member = cont.findMember(qName);
    		if (member==null) return null;
    		cont = member.getVariableType().getType();
    	}
    	return cont.findMember(what.get(what.size()-1));
    }*/
            ...
            if(el instanceof MethodCall) {
                ...
                if(method.getBody() instanceof CompoundStatement) {
                    ...
                    if(cs.getElements().get(0) instanceof JumpStatement) {
                        ...
                        result = createHelper(actualContext.context, actualContext.what, js.getExpression(),
    ...
    public Transaction getAddInitTransaction(String init, boolean addContainers) {
        ...
        String ins = pre + tabs(what.size()) + what.get(size - 1) + (isArray() ? " += {" : " = ") + containers + init
                + (isArray() ? "}" : "") + ";\r\n" + post;
    ...
    private void parseJoinExpression(Init2 init, List<Init2> tableInits, List<String> lWhere, List<String> rWhere,
            List<IPosition> expressions) {
    ...
    private StringTree parseDataSource(InitHelper h, Map<InitHelper, StringTree> tableInits, List<String> lWhere,
            List<String> rWhere, List<IPosition> expressions) {
    ...
    private void reduceTree(StringTree tree, List<InitHelper> neededHelpers) {
        ...
        for(Entry<String, StringTree> e : tree.getChildren().entrySet()) {
            ...
            if(!neededHelpers.contains(new InitHelper(context).add(e.getValue().name()))
                    && e.getValue().getChildren().isEmpty()) {
    ...
    public StringTree getTablesTree() {
        ...
        if(inits1.size() > 0) {
            // Используется новая парадигма fromJoin = ...
            ...
            for (InitHelper hhe : helpers.keySet()){
            	Init2 init = hhe.findInitDeep();
            	if (init.element instanceof IType) {
            		IType t = (IType) init.element;
            ...
            IType curr = getType();
            // парсим where по всем родительским классам и нашему классу
            while(curr != null) {
        ...
        for(int i = 0; i < lWhere.size(); i++) {
            ...
            for (int j=0; j<right.length(); j++)
            	if (right.charAt(j)=='.')
            		pos=j;
            if (pos==-1) continue;
            String parentTable = right.substring(0,pos);
            String childRelation = right.substring(pos+1);*/
    ...
    public boolean deleteObject(MultipleTransactions mt, Shell shell) {
        ...
        String message = "Хотите ли Вы удалить объект " + toQname() + "?";
        return deleteCode(offset, length, message, doSwitch, mt, shell, true);
    }

    public static boolean deleteCode(int offset, int length, String message, boolean doSwitch, MultipleTransactions mt,
            Shell shell, boolean lookForEndStatement) {
        ...
        if(doSwitch)
            message += "\r\nВыделенный код будет удалён.";
        MessageDialog d = new MessageDialog(shell, "Удаление объекта", null, message, MessageDialog.QUESTION, new String[] {
                "Да", "Нет", "Отмена" }, 0);
    ...
    public static boolean deleteLanguageElement(ILanguageElement el, String message, boolean doSwitch,
            MultipleTransactions mt, Shell shell) {
    ...
    public int getNewInitPosition(IDocument document, String name) {
        ...
        for(IInitializer m : context.getInitializers()) {
            if(!badInits.contains(m)) {
                if(name == null
                        || (name.compareToIgnoreCase(m.getLeftName()) > 0 && (last == null || last.getLeftName()
                                .compareToIgnoreCase(m.getLeftName()) < 0))) {
                    last = m;
                }
            }
        }
    ...
    public int getNewMethodPosition(IDocument document, String name) {
        ...
        for(IMethod m : context.getMethods()) {
            if(name.compareToIgnoreCase(m.getName()) > 0
                    && (last == null || last.getName().compareToIgnoreCase(m.getName()) < 0)) {
                last = m;
            }
        }
        if(last != null) {
