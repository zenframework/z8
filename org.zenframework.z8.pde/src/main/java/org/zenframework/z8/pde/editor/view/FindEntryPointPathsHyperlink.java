...
public class FindEntryPointPathsHyperlink implements IHyperlink {
        ...
        if(from.equals(to)) {
            ...
            if(size > 0) {
                ...
                for(int i = 1; i < size; i++)
                    msg = msg + m_members.get(i - 1).getVariableType().getSignature() + " " + m_members.get(i - 1).getName()
                            + SEP;
                msg = msg + to.getUserName() + " " + m_members.get(size - 1).getName() + ";";
            }
            else
                msg = "[" + from.getCompilationUnit().getPath() + "] " + from.getUserName() + " - точка входа;";
            msgs.add(msg);
            return;
        }
    ...
    public void open() {
        ...
        for(String m : msgs)
            msg = msg + m + "\r\n";
        if(msg.equals(""))
            msg = "Путей не найдено.";
        MessageDialog.openInformation(null, "Поиск путей от точек входа до класса " + to.getUserName() + ".", msg);
    }
}
