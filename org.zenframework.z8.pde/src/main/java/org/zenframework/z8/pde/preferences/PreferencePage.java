...
public class PreferencePage extends org.eclipse.jface.preference.PreferencePage implements IWorkbenchPreferencePage {
    ...
    protected Control createContents(Composite parent) {
        ...
        //SWTUtil.createPushButton(composite, "&Browse...", null); //$NON-NLS-1$
    ...
    private void initializeValues() {
        ...
        txt_tomcatMemoryBound.setText(new Integer(doGetPreferenceStore().getInt(
                PreferencePageConsts.ATTR_TOMCAT_MEMORY_BOUND)).toString());
    ...
    private void validatePage() {
        ...
        if(!tomcatMemoryBound.matches("[0-9]*")) {
            setErrorMessage("¬ведите число в поле Tomcat Memory Bound");
            setValid(false);
            return;
        }
        ...
        if(tomcatHome.length() > 0) {
            if(!new File(tomcatHome).exists()) {
                setErrorMessage("”казан не существующий путь к Tomcat Home");
                setValid(false);
                return;
            }
        }
        ...
        if(webInfPath.length() > 0) {
            if(!new File(webInfPath).exists()) {
                setErrorMessage("”казан не существующий путь к WebInf");
                setValid(false);
                return;
            }
        }
    ...
    private void storeValues() {
        ...
        doGetPreferenceStore().setValue(PreferencePageConsts.ATTR_TOMCAT_MEMORY_BOUND,
                new Integer(txt_tomcatMemoryBound.getText()).intValue());
