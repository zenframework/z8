...
public class Model extends ObjectWithProperties implements IPropertySource, IMergeable {
    ...
    private class AttributedPropertySource implements IPropertySource {
        ...
        public Object getPropertyValue(Object id) {
            //IType t = getHelper().context.getCompilationUnit().getReconciledType();
        ...
        public void resetPropertyValue(Object id) {
            ...
            if(dp == null)
                return;
            //if (getHelper().context!=getHelper().context.getCompilationUnit().getReconciledType()) return;
            AttributedProperty att = (AttributedProperty)id;
            //unset boolean
        ...
        public void setPropertyValue(Object id, Object value) {
            ...
            //set attribute
                ...
                else if(attributed.size() > 0
                        && ((ILanguageElement)attributed.get(0)).getCompilationUnit() == getHelper().context
                                .getCompilationUnit()) {
    ...
    public static IPropertyDescriptor createBooleanAttributePropertyDescriptor(String attribute) {
        return new ComboBoxPropertyDescriptor(new AttributedProperty(attribute, true), "[" + attribute + "]", new String[] {
                "false", "true" });
    }
    ...
    public List<IPropertyDescriptor> collectAttributeProperties() {
        ...
        ((TextPropertyDescriptor)d).setValidator(new ICellEditorValidator() {
            ...
            public String isValid(Object value) {
                ...
                if(!namedOk)
                    return "Плохое значение для атрибута named.";
                return null;
            }
    ...
    private IPropertyDescriptor m_attributedPropertyDescriptor = new PropertyDescriptor(m_attributedId, "[Атрибуты]");
    ...
    public void resetPropertyValue(Object id) {
        if(id instanceof Property) {
            ...
            if(init != null && init.element != null
                    && init.element.getCompilationUnit() == getHelper().context.getCompilationUnit()) {
                    ...
                    if(d.getChar(pos) == '=') {
                        ...
                        if(Character.isJavaIdentifierPart(d.getChar(pos))) {
                            ...
                            while(Character.isWhitespace(d.getChar(pos)))
                                pos--;
                            if(!Character.isJavaIdentifierPart(d.getChar(pos))) {
                                int end = init.element.getSourceRange().getOffset()
                                        + init.element.getSourceRange().getLength();
    ...
    public void setPropertyValue(Object id, Object value) {
        ...
        if(id instanceof Property) {
            ...
            if(init != null && init.element != null
                    && init.element.getCompilationUnit() == getHelper().context.getCompilationUnit()) {
                mt.add(new Transaction(init.element.getSourceRange().getLength(), init.element.getSourceRange().getOffset(),
                        resultString));
            }
            else {
    ...
    public static IPropertyDescriptor createBooleanPropertyDescriptor(String name) {
        return new ComboBoxPropertyDescriptor(new Property(PROPERTY_TYPE_BOOLEAN, name), name, new String[] { "false",
                "true" });
    }
