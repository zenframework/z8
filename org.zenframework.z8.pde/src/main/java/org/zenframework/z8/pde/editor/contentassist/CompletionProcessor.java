...
public class CompletionProcessor implements IContentAssistProcessor {
    ...
    protected final static String[] fgProposals = { "binary", "bool", "break", "catch", "class", "container", "continue",
            "date", "datetime", "do", "decimal", "else", "extends", "false", "finally", "for", "guid", "if", "int",
            "native", "new", "null", "private", "protected", "public", "return", "static", "super", "this", "throw", "true",
            "try", "void", "while" };
    ...
    private String getCharsAfterDot(ITextViewer v, int off) {
        int i = 0;
        int of = off - 1;
        char[] chars = new char[128]; //должно хватить
        try {
    ...
    private ICompletionProposal[] getProposals(IVariableType variableType, int documentOffset) {
        ...
        for(IMember member : members) {
            ...
            if(member instanceof IMethod) {
                ...
                String text = method.getUserName() + " " + method.getVariableType().getSignature() + " - "
                        + variableType.getSignature();
                IContextInformation info = new ContextInformation(text, text);
                int mv = method.getParametersCount() > 0 ? 1 : 2;
                ICompletionProposal proposal = new CompletionProposal(method.getName() + "()", documentOffset, 0, method
                        .getName().length() + mv, leip.getImageLabel(member, LanguageElementImageProvider.SMALL_ICONS),
                        text, info, text);
                ...
                IContextInformation info = new ContextInformation(text, text);
                ICompletionProposal proposal = new CompletionProposal(member.getName(), documentOffset, 0, member.getName()
                        .length(), leip.getImageLabel(member, LanguageElementImageProvider.SMALL_ICONS), text, info, text);
    ...
    public IContextInformation[] computeContextInformation(ITextViewer viewer, int documentOffset) {
        ...
        for(int i = 0; i < result.length; i++)
            result[i] = new ContextInformation(
                    MessageFormat
                            .format(Z8EditorMessages.getString("CompletionProcessor.ContextInfo.display.pattern"), new Object[] { new Integer(i), new Integer(documentOffset) }), //$NON-NLS-1$
                    MessageFormat.format(
                            Z8EditorMessages.getString("CompletionProcessor.ContextInfo.value.pattern"), new Object[] { new Integer(i), new Integer(documentOffset - 5), new Integer(documentOffset + 5) })); //$NON-NLS-1$
        return result;
    }
