...
public abstract class JDXReferenceType extends JDXType {
    ...
    /*
    	public IVariable getField(String name) throws DebugException
    	{
    		try
    		{
    			...
    			{
    				return new JDXFieldVariable(getDebugTarget(), getJDXThread(), field, type);
    			}
    		}
    		catch(RuntimeException e)
    		{
    			getDebugTarget().targetRequestFailed(MessageFormat.format(JDXMessages.JDXClassType_exception_while_retrieving_field, new Object[] { e.toString(), name }), e);
    		}
    		return null;
    	}
    */
    public JDXClassObjectValue getClassObject() throws DebugException {
        try {
            ReferenceType type = (ReferenceType)getUnderlyingType();
            return (JDXClassObjectValue)JDXValue.createValue(getDebugTarget(), getJDXThread(), getJDXVariable(),
                    type.classObject());
        }
        catch(RuntimeException e) {
            getDebugTarget().targetRequestFailed(
                    MessageFormat.format(JDXMessages.JDXClassType_exception_while_retrieving_class_object,
    ...
    public IValue getClassLoaderObject() throws DebugException {
        try {
            ...
            getDebugTarget().targetRequestFailed(
                    MessageFormat.format(JDXMessages.JDXReferenceType_0, new Object[] { e.toString() }), e);
        }
        return null;
    }
    ...
    static public String getTypeName(String genericTypeSignature) {
        ...
        if(parameterStart < 0) {
            ...
            name.append(Signature.toString(genericTypeSignature).substring(parameterStart - 1 - arrayDimension)
                    .replace('/', '.'));
        }
    ...
    static protected String getUserTypeName(String typeName) {
        ...
        if(isArray(typeName)) {
            ...
            if(bracket != -1)
                typeName = getUserTypeName(unwrap(typeName.substring(BuiltinNative.ArrayQualifiedName.length() + 1,
                        typeName.length() - 1)));
            else
                typeName = "value";
