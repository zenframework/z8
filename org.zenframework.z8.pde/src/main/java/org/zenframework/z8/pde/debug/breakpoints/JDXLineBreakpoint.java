...
public class JDXLineBreakpoint extends JDXBreakpoint implements ILineBreakpoint {
    ...
    public JDXLineBreakpoint(IResource resource, String typeName, int lineNumber, int javaLineNumber, int charStart,
            int charEnd, int hitCount, boolean add, Map<String, Object> attributes) throws DebugException {
        this(resource, typeName, lineNumber, javaLineNumber, charStart, charEnd, hitCount, add, attributes,
                JDX_LINE_BREAKPOINT);
    }

    protected JDXLineBreakpoint(final IResource resource, final String typeName, final int lineNumber,
            final int javaLineNumber, final int charStart, final int charEnd, final int hitCount, final boolean add,
            final Map<String, Object> attributes, final String markerType) throws DebugException {
        IWorkspaceRunnable wr = new IWorkspaceRunnable() {
            @Override
            public void run(IProgressMonitor monitor) throws CoreException {
                setMarker(resource.createMarker(markerType));
                addLineBreakpointAttributes(attributes, getModelIdentifier(), true, lineNumber, javaLineNumber, charStart,
                        charEnd);
    ...
    public int getJavaLineNumber() throws CoreException {
        ...
        //return ensureMarker().getResource().getAttribute(JAVA_LINE_NUMBER, -1);
    }
    ...
    protected BreakpointRequest createLineBreakpointRequest(Location location, JDXDebugTarget target) throws CoreException {
        ...
        if(manager == null) {
            target.requestFailed(JDXMessages.JDXLineBreakpoint_Unable_to_create_breakpoint_request___VM_disconnected__1,
                    null);
        }
        try {
    ...
    protected List<Location> determineLocations(int lineNumber, ReferenceType type, JDXDebugTarget target) {
        ...
        try {
            ...
            IStatus status = new Status(IStatus.ERROR, Plugin.getUniqueIdentifier(), NO_LINE_NUMBERS,
                    JDXMessages.JDXLineBreakpoint_Absent_Line_Number_Information_1, null);
    ...
    public void addLineBreakpointAttributes(Map<String, Object> attributes, String modelIdentifier, boolean enabled,
            int lineNumber, int javaLineNumber, int charStart, int charEnd) {
    ...
    protected String getMarkerMessage(boolean conditionEnabled, String condition, int hitCount, int suspendPolicy,
            int lineNumber) {
        StringBuffer message = new StringBuffer(super.getMarkerMessage(hitCount, suspendPolicy));
        if(lineNumber != -1) {
            message.append(MessageFormat.format(JDXMessages.JDXLineBreakpoint___line___0___1,
                    new Object[] { Integer.toString(lineNumber) }));
        }
        if(conditionEnabled && condition != null) {
            message.append(MessageFormat.format(JDXMessages.JDXLineBreakpoint___Condition___0___2,
