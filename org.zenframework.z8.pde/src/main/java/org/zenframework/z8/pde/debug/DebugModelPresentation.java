...
public class DebugModelPresentation extends LabelProvider implements IDebugModelPresentation {
    ...
    protected void appendInstanceFilter(JDXBreakpoint breakpoint, StringBuffer buffer) throws CoreException {
        ...
        for(int i = 0; i < instances.length; i++) {
            ...
            if(instanceText != null) {
                buffer.append(' ');
                buffer.append(MessageFormat.format(JDXMessages.JDXModelPresentation_instance_1,
                        new Object[] { instanceText }));
            }
        }
    }
    ...
    protected String getThreadText(JDXThread thread) throws CoreException {
        if(thread.isTerminated()) {
            if(thread.isSystemThread()) {
                return getFormattedString(JDXMessages.JDXModelPresentation_System_Thread____0____Terminated__7,
                        thread.getName());
            }
        ...
        if(thread.isStepping()) {
            if(thread.isSystemThread()) {
                return getFormattedString(JDXMessages.JDXModelPresentation_System_Thread___0____Stepping__9,
                        thread.getName());
            }
        ...
        if(!thread.isSuspended() || (thread.isSuspendedQuiet())) {
            if(thread.isSystemThread()) {
                return getFormattedString(JDXMessages.JDXModelPresentation_System_Thread___0____Running__11,
                        thread.getName());
            }
        ...
        if(breakpoints.length > 0) {
            ...
            if(breakpoint instanceof JDXExceptionBreakpoint) {
                ...
                if(thread.isSystemThread()) {
                    return getFormattedString(
                            JDXMessages.JDXModelPresentation_System_Thread___0____Suspended__exception__1____13,
                            new String[] { thread.getName(), exName });
                }
                return getFormattedString(JDXMessages.JDXModelPresentation_Thread___0____Suspended__exception__1____14,
                        new String[] { thread.getName(), exName });
            }
            ...
            if(breakpoint instanceof JDXWatchpoint) {
                ...
                if(wp.isAccessSuspend(thread.getDebugTarget())) {
                    if(thread.isSystemThread()) {
                        return getFormattedString(
                                JDXMessages.JDXModelPresentation_System_Thread___0____Suspended__access_of_field__1__in__2____16,
                                new String[] { thread.getName(), fieldName, typeName });
                    }
                    return getFormattedString(
                            JDXMessages.JDXModelPresentation_Thread___0____Suspended__access_of_field__1__in__2____17,
                            new String[] { thread.getName(), fieldName, typeName });
                }
                // modification
                if(thread.isSystemThread()) {
                    return getFormattedString(
                            JDXMessages.JDXModelPresentation_System_Thread___0____Suspended__modification_of_field__1__in__2____18,
                            new String[] { thread.getName(), fieldName, typeName });
                }
                return getFormattedString(
                        JDXMessages.JDXModelPresentation_Thread___0____Suspended__modification_of_field__1__in__2____19,
                        new String[] { thread.getName(), fieldName, typeName });
            }
            ...
            if(breakpoint instanceof JDXMethodBreakpoint) {
                ...
                if(me.isEntrySuspend(thread.getDebugTarget())) {
                    if(thread.isSystemThread()) {
                        return getFormattedString(
                                JDXMessages.JDXModelPresentation_System_Thread___0____Suspended__entry_into_method__1__in__2____21,
                                new String[] { thread.getName(), methodName, typeName });
                    }
                    return getFormattedString(
                            JDXMessages.JDXModelPresentation_Thread___0____Suspended__entry_into_method__1__in__2____22,
                            new String[] { thread.getName(), methodName, typeName });
                }
                if(thread.isSystemThread()) {
                    return getFormattedString(
                            JDXMessages.JDXModelPresentation_System_Thread___0____Suspended__exit_of_method__1__in__2____21,
                            new String[] { thread.getName(), methodName, typeName });
                }
                return getFormattedString(
                        JDXMessages.JDXModelPresentation_Thread___0____Suspended__exit_of_method__1__in__2____22,
                        new String[] { thread.getName(), methodName, typeName });
            }
                ...
                if(lineNumber > -1) {
                    if(thread.isSystemThread()) {
                        if(isRunToLineBreakpoint(jlbp)) {
                            return getFormattedString(
                                    JDXMessages.JDXModelPresentation_System_Thread___0____Suspended__run_to_line__1__in__2____23,
                                    new String[] { thread.getName(), String.valueOf(lineNumber), typeName });
                        }
                        return getFormattedString(
                                JDXMessages.JDXModelPresentation_System_Thread___0____Suspended__breakpoint_at_line__1__in__2____24,
                                new String[] { thread.getName(), String.valueOf(lineNumber), typeName });
                    }
                    if(isRunToLineBreakpoint(jlbp)) {
                        return getFormattedString(
                                JDXMessages.JDXModelPresentation_Thread___0____Suspended__run_to_line__1__in__2____25,
                                new String[] { thread.getName(), String.valueOf(lineNumber), typeName });
                    }
                    return getFormattedString(
                            JDXMessages.JDXModelPresentation_Thread___0____Suspended__breakpoint_at_line__1__in__2____26,
                            new String[] { thread.getName(), String.valueOf(lineNumber), typeName });
                }
            }
            if(breakpoint instanceof JDXClassPrepareBreakpoint) {
                return getFormattedString(JDXMessages.JDXModelPresentation_115, new String[] { thread.getName(),
                        getTypeName(breakpoint.getTypeName()) });
            }
        }
        if(thread.isSystemThread()) {
    ...
    protected String getValueCharText(JDXValue value) throws DebugException {
        ...
        if(Character.getType(charValue) == Character.CONTROL) {
            ...
            case 0:
                charText.append(" (NUL)");break; //$NON-NLS-1$
            case 8:
                charText.append(" (BS)");break; //$NON-NLS-1$
            case 9:
                charText.append(" (TAB)");break; //$NON-NLS-1$
            case 10:
                charText.append(" (LF)");break; //$NON-NLS-1$
            case 13:
                charText.append(" (CR)");break; //$NON-NLS-1$
            case 21:
                charText.append(" (NL)");break; //$NON-NLS-1$
            case 27:
                charText.append(" (ESC)");break; //$NON-NLS-1$
            case 127:
                charText.append(" (DEL)");break; //$NON-NLS-1$
            }
        }
        else {
