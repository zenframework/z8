...
public class OpenSourceAction extends Action {
    ...
    public void run() {
        ...
        try {
            ...
            IEditorDescriptor ed = PlatformUI.getWorkbench().getEditorRegistry()
                    .findEditor("org.zenframework.z8.forms.editors.MultiEditor");
            if(ed != null) {
                ...
                multi.getEditorSite().getSelectionProvider()
                        .setSelection(new TextSelection(pos.getOffset(), pos.getLength()));
            }
            else {
                IEditorPart ep = IDE.openEditor(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage(),
                        (IFile)file, "org.zenframework.z8.pde.Z8Editor");
