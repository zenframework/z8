...
public class LanguageElementLabelProvider extends LabelProvider {
    ...
    private void updateTextProviderFlags() {
        ...
        if(getFlag(SHOW_QUALIFIED)) {
            m_textFlags |= (LanguageElementLabels.F_FULLY_QUALIFIED | LanguageElementLabels.M_FULLY_QUALIFIED
                    | LanguageElementLabels.I_FULLY_QUALIFIED | LanguageElementLabels.T_FULLY_QUALIFIED
                    | LanguageElementLabels.D_QUALIFIED | LanguageElementLabels.CF_QUALIFIED | LanguageElementLabels.CU_QUALIFIED);
        }
        if(getFlag(SHOW_POST_QUALIFIED)) {
            m_textFlags |= (LanguageElementLabels.F_POST_QUALIFIED | LanguageElementLabels.M_POST_QUALIFIED
                    | LanguageElementLabels.I_POST_QUALIFIED | LanguageElementLabels.T_POST_QUALIFIED
                    | LanguageElementLabels.D_POST_QUALIFIED | LanguageElementLabels.CF_POST_QUALIFIED | LanguageElementLabels.CU_POST_QUALIFIED);
        }
    }
