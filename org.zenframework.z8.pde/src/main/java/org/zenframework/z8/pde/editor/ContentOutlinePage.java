...
public class ContentOutlinePage extends org.eclipse.ui.views.contentoutline.ContentOutlinePage {
    ...
    public void selectionChanged(SelectionChangedEvent event) {
        ...
        if(element instanceof ISource) {
            ...
            m_editor.getSelectionProvider().setSelection(
                    new TextSelection(source.getPosition().getOffset(), source.getPosition().getLength()));
        }
        return;
    ...
    public void makeContributions(IMenuManager menuManager, IToolBarManager toolBarManager,
        ...
        menuManager.addMenuListener(new IMenuListener() {
            ...
            public void menuAboutToShow(final IMenuManager manager) {
                manager.removeAll();
                final CompilationUnit unit = Workspace.getInstance().getCompilationUnit(
                        ((FileEditorInput)m_editor.getEditorInput()).getFile());
                                ...
                                actions.add(new Action(method) {
                                    ...
                                    public void run() {
                                        ...
                                        methodString = methodString + "\r\n\t\t"
                                                + (m.getVariableType().getSignature().equals("void") ? "" : "return ")
                                                + call;
                                        ...
                                        methodString += "\r\n\t}";
                                        int posToInsert = new InitHelper(type).getNewMethodPosition(
                                                ((Z8Editor)m_editor).getDocument(), m.getName());
                                        mt.add(0, posToInsert, methodString);
                                        mt.execute();

                                        m_editor.setFocus();
                                        m_editor.getSelectionProvider()
