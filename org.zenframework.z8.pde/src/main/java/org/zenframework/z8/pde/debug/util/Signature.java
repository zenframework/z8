package org.zenframework.z8.pde.debug.util;
 ...
 * to signatures: type variables, parameterized types with type arguments, and
 * formal type parameters. <it>Rich</it> signatures containing these facets only
 * occur when dealing with code that makes overt use of the new language
 ...
 * </p>
 * <p>
 * Note that the "Q" and "!" formats are specific to Eclipse; the remainder are
 ...
 * <li><code>"Ljava.lang.String;"</code> denotes <code>java.lang.String</code>
 * in compiled code</li>
 * <li><code>"QString;"</code> denotes <code>String</code> in source code</li>
 * <li><code>"Qjava.lang.String;"</code> denotes <code>java.lang.String</code>
 * in source code</li>
 * <li><code>"[QString;"</code> denotes <code>String[]</code> in source
 * code</li>
 * <li><code>"QMap&lt;QString;&ast;&gt;;"</code> denotes
 ...
 * <li><code>"(QString;)QObject;"</code> denotes <code>Object foo(String)</code>
 * in source code</li>
 * </ul>
 * </p>
 * <p>
 ...
 * <li><code>"X:QReader;"</code> denotes <code>X extends Reader</code> in source
 * code</li>
...
public final class Signature {
	 ...
	 * signature. Value is <code>'&ast;'</code>.
	 * 
	 * @since 3.0
	 */
	 ...
	 *         {@link #TYPE_VARIABLE_SIGNATURE}, or (since 3.1)
	...
	public static char[][] getThrownExceptionTypes(char[] methodSignature) throws IllegalArgumentException {
		...
		if(exceptionStart == -1) {
			...
			// ignore return type
			exceptionStart = scanTypeSignature(methodSignature, paren + 1) + 1;
	...
	/**
	 * Extracts the type parameter signatures from the given method or type
	 * signature. The method or type signature is expected to be dot-based.
	 * 
		...
		try {
			...
			while(i < length) {
				...
				while(methodOrTypeSignature[i] == ':') {
					...
					case C_RESOLVED:
						try {
							i = scanClassTypeSignature(methodOrTypeSignature, i);
							i++; // position at start of next param if any
						} catch(IllegalArgumentException e) {
							// not a class type signature -> it is a new type
							// parameter
						}
						break;
					case C_ARRAY:
						try {
							i = scanArrayTypeSignature(methodOrTypeSignature, i);
							i++; // position at start of next param if any
						} catch(IllegalArgumentException e) {
							// not an array type signature -> it is a new type
							// parameter
						}
						break;
					case C_TYPE_VARIABLE:
						try {
							i = scanTypeVariableSignature(methodOrTypeSignature, i);
							i++; // position at start of next param if any
						} catch(IllegalArgumentException e) {
							// not a type variable signature -> it is a new type
							// parameter
						}
						break;
	...
	public static String getSimpleName(String name) {
		...
		buffer.append(nameChars, lastGenericEnd + 1, length - lastGenericEnd - 1); // copy
																					// trailing
																					// portion,
																					// may
																					// contain
	 ...
	 *            the string buffer to append to
	 * @param isVarArgs
	 *            <code>true</code> if the type must be displayed as a variable
	 *            argument, <code>false</code> otherwise. In this case, the type
