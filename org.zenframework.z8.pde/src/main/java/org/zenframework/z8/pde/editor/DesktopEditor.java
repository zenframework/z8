...
public class DesktopEditor extends EditorPart implements ResourceListener {
    ...
    public void createPartControl(Composite parent) {
        ...
        label.setText("Дерево рабочих столов:");
        ...
        tree.addDropSupport(DND.DROP_DEFAULT | DND.DROP_MOVE | DND.DROP_COPY,
                new Transfer[] { LocalSelectionTransfer.getTransfer() }, new DropTargetListener() {
                    @Override
        ...
        buttonAddObject.setText("Добавить объект");
        buttonAddObject.addSelectionListener(new SelectionListener() {
            ...
            public void widgetSelected(SelectionEvent e) {
                TypeSelectDialog dialog = new TypeSelectDialog(getSite().getShell(), compilationUnit.getProject(),
                        "Runnable", "Выберите объект", "Выберите класс объекта:");
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                for(Object o : ts.toArray()) {
                    ...
                    if(!mem.getCompilationUnit().getReconciledType().equals(t)) {
                        MessageDialog d = new MessageDialog(editor.getSite().getShell(), "Удаление", null,
                                "Невозможно удалить объект " + m.getHelper().toQname()
                                        + ", т.к. он описан в классе, отличном от редактируемого", MessageDialog.ERROR,
                                new String[] { "ОК" }, 0);
                        d.open();
                        return;
                    }
        ...
        buttonUp.addSelectionListener(new SelectionListener() {
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                for(Object o : ts.toArray()) {
                    ...
                    IMember mem = m.getHelper().getMember();
                    if(!mem.getCompilationUnit().getReconciledType().equals(t)) {
                        MessageDialog d = new MessageDialog(editor.getSite().getShell(), "Перемешение", null,
                                "Невозможно переместить объект " + m.getHelper().toQname()
                                        + ", т.к. он описан в классе, отличном от редактируемого", MessageDialog.ERROR,
                                new String[] { "ОК" }, 0);
                        d.open();
                        return;
                    ...
                    if(prev == null) {
                        MessageDialog d = new MessageDialog(editor.getSite().getShell(), "Перемешение", null,
                                "Невозможно переместить объект " + m.getHelper().toQname() + " вверх, т.к. он и так первый",
                                MessageDialog.ERROR, new String[] { "ОК" }, 0);
                        d.open();
                        return;
                    }
                    sw.put(prev, mem);
                ...
                for(IMember key : sw.keySet()) {
                    ...
                    try {
                        tr.add(posKey.getLength(), posKey.getOffset(),
                                editor.getDocument().get(valKey.getOffset(), valKey.getLength()));
                        tr.add(valKey.getLength(), valKey.getOffset(),
                                editor.getDocument().get(posKey.getOffset(), posKey.getLength()));
                    }
                    catch(Exception e1) {
                        Plugin.log(e1);
                    }
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                for(Object o : ts.toArray()) {
                    ...
                    if(next == null) {
                        MessageDialog d = new MessageDialog(editor.getSite().getShell(), "Перемешение", null,
                                "Невозможно переместить объект " + m.getHelper().toQname()
                                        + " вниз, т.к. он и так последний", MessageDialog.ERROR, new String[] { "ОК" }, 0);
                        d.open();
                        return;
                    }
                    sw.put(mem, next);
