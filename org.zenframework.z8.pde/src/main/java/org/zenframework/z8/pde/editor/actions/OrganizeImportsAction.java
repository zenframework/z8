...
public class OrganizeImportsAction extends TextEditorAction {
    ...
    static {
        try {
            RESOURCE_IMAGE = new Image((Device)null, FileLocator.openStream(Plugin.getDefault().getBundle(), new Path(
                    "icons/obj16/z8.gif"), true));
        }
        catch(Exception e) {
            Plugin.log(e);
        }
    ...
    public void run() {
        ...
        if(validateEditorInputState()) {
            try {
                if(importBlock == null) {
                    getDocument().replace(0, 0, imports + (imports.length() > 0 ? "\r\n" : ""));
                }
                else {
                    getDocument().replace(importBlock.getPosition().getOffset(), importBlock.getPosition().getLength(),
                            imports);
                }
            }
