...
public class JavaBuilder {
    ...
    private class PrepareForRun {
        ...
        PrepareForRun(IProject iProject, String memoryBound, boolean useJar) {
            ...
            IPath classes = m_useJar ? new Path(BuildPathManager.getJavaOutputPath(m_iProject).toString() + "/../classes")
                    : m_webInf.append("/classes");
            ...
            for(IProject proj : getAllReferencedProjectsFromMain(m_project)) {
                ...
                if(ProjectDir.canRead()) {
                    ...
                    for(File xml_file : getFileListingIgnoreSVNwithFilterNoSort(ProjectDir)) {
                        try {
                            FileUtils.copyFile(new Path(xml_file.getPath()), resourcesPath.append(xml_file.getName()),
                                    true);
                        }
                        catch(IOException e) {
                            e.printStackTrace();
                    ...
                    for(File rpt_file : getFileListingIgnoreSVNwithFilterNoSort(ProjectDir)) {
                        try {
                            FileUtils.copyFile(new Path(rpt_file.getPath()),
                                    staticReportsExploded.append(rpt_file.getName()), true);
                        }
                        catch(IOException e) {
                            e.printStackTrace();
                        }
                    ...
                    for(File reportbinding_file : getFileListingIgnoreSVNwithFilterNoSort(ProjectDir)) {
                        try {
                            FileUtils.copyFile(new Path(reportbinding_file.getPath()),
                                    staticReportsExploded.append(reportbinding_file.getName()), true);
                        }
                        catch(IOException e) {
                            e.printStackTrace();
                        }
    ...
    boolean compareSourceVsTarget(IPath sourcePath, IPath targetPath) {
        ...
        try {
            ...
            sourceFile_truename = canonical_form.substring(canonical_form.lastIndexOf(File.separator) + 1,
                    canonical_form.lastIndexOf('.'));
            canonical_form = targetFile.getCanonicalPath();
            targetFile_truename = canonical_form.substring(canonical_form.lastIndexOf(File.separator) + 1,
                    canonical_form.lastIndexOf('.'));
        }
        catch(IOException e) {
            Plugin.log(e);
        ...
        // для проверки случая если различается только регистр некоторых букв в имени java-файла и class-файла (exists() не различает регистр!!)
        if(!sourceFile_truename.equals(targetFile_truename)) {// если есть различие, то class-файл от старой версии java-файла => удаляем все такие class-файлы !!
    ...
    private String[] getJavaFilesList(Project project, IPath javaClasses) {
        ...
        for(CompilationUnit compilationUnit : dependencies) {
            if(!compilationUnit.containsNativeType()) {
                ...
                IPath targetPath = javaClasses.append(compilationUnit.getPackagePath())
                        .append(compilationUnit.getSimpleName()).addFileExtension("class");
    ...
    static public void run(IProject iProject, boolean isDebugMode, String memoryBound, boolean useJar) throws IOException,
            JavaBuilderException, RepositoryLocalChangesException {
        new JavaBuilder(iProject).build(isDebugMode, memoryBound, useJar);
    }

    protected void build(boolean isDebugMode, String memoryBound, boolean useJar) throws IOException, JavaBuilderException,
            RepositoryLocalChangesException {
