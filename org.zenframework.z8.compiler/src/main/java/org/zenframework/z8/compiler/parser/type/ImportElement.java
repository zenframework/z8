...
public class ImportElement extends LanguageElement {
    ...
    public boolean resolveTypes(CompilationUnit compilationUnit, IType declaringType) {
        ...
        if(importedUnit == null) {
            setError(
                    qualifiedName.getFirstToken().getPosition()
                            .union(qualifiedName.getTokens()[lastResolvedToken].getPosition()), "The import "
                            + qualifiedName.toString(0, lastResolvedToken + 1) + " cannot be resolved");
            return false;
        }
    ...
    public void replaceImport(TextEdit parent, IPath oldImport, IPath newImport) {
        ...
        if(qName.equals(oldImport.toString().replace(IPath.SEPARATOR, '.'))) {
            ...
            parent.addChild(new ReplaceEdit(position.getOffset(), position.getLength(), newImport.toString().replace(
                    IPath.SEPARATOR, '.')));
        }
    }
}
