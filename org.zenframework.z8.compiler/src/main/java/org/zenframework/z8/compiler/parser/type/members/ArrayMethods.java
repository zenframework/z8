...
public class ArrayMethods {
	static public IMethod[] get(CompilationUnit compilationUnit, IVariableType variableType) {
		...
		if(keyType == null) // array methods
		{
			...
			{
				Variable[] parameters = new Variable[] { new Variable(new VariableType(compilationUnit, integerType), "index"), new Variable(variableType, "elements") };
				methods.add(new Method(new VariableType(compilationUnit, voidType), "addAll", parameters));
			}

			// TYPE removeAt(int index)
			{
				Variable[] parameters = new Variable[] { new Variable(new VariableType(compilationUnit, integerType), "index") };
...
*/
			...
			{
				Variable[] parameters = new Variable[] { new Variable(variableType, "elements") };
				methods.add(new Operator(new VariableType(compilationUnit, voidType), new OperatorToken(IToken.ADD_ASSIGN, null), parameters));
			}
