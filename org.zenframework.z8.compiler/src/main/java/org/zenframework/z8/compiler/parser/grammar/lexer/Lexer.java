...
public class Lexer {
    ...
    public Token nextToken() throws TokenException {
        ...
        while(true) {
            ...
            if(skipSpaces) {
                if(token.getId() == IToken.COMMENT || token.getId() == IToken.WHITESPACE
                        || token.getId() == IToken.LINEBREAK) {
                    continue;
                }
    ...
    Token getDateTime(IPosition position) {
        ...
        try {
            ...
            if(bHasDate) {
                ...
                if(nYear < 1 || nYear > 9999 || nMonth < 1 || nMonth > 12 || nDay < 1
            ...
            if(bHasDate) {
                if(bHasTime) {
                    return new DatetimeToken(new Datetime(nYear, nMonth, nDay, nHour, nMinute, nSecond), new Position(start,
                            position));
                }
    ...
    boolean isNumberSign(IPosition position) {
        ...
        if(currentToken.getId() != IToken.NOTHING && !(currentToken instanceof KeywordToken)
                || currentToken.getId() == IToken.RBRACE)
            return false;
