...
public class LoopCondition extends LanguageElement {
	...
	public boolean checkSemantics(CompilationUnit compilationUnit, IType declaringType, IMethod declaringMethod, IVariable leftHandValue, IVariableType context) {
		...
		if(expression != null && booleanType != null && expression.checkSemantics(compilationUnit, declaringType, declaringMethod, null, null)) {
			typeCast = expression.getVariableType().getCastTo(booleanType);

			if(typeCast == null) {
				setError(expression.getPosition(), "Type mismatch: cannot convert from " + expression.getVariableType().getSignature() + " to " + booleanType.getUserName());
				return false;
			}
		}
