...
public class MapElement extends LanguageElement {
	...
	public boolean containsQualifiedName(String name) {
		return key.containsQualifiedName(name) || value.containsQualifiedName(name);
	}
	...
	public boolean checkSemantics(CompilationUnit compilationUnit, IType declaringType, IMethod declaringMethod, IVariable leftHandValue, IVariableType context) {
		...
		if(!key.checkSemantics(compilationUnit, declaringType, declaringMethod, null, null) || !value.checkSemantics(compilationUnit, declaringType, declaringMethod, new Variable(valueType), null))
			return false;

		if(key.getVariableType().isArray()) {
			setError(key.getPosition(), "Array (map) type " + key.getVariableType().getSignature() + " cannot be used as a map key type");
			return false;
		}
