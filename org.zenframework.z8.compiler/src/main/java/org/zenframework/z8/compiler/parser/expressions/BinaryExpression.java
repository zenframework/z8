...
public class BinaryExpression extends LanguageElement {
    ...
    public boolean resolveTypes(CompilationUnit compilationUnit, IType declaringType) {
        return super.resolveTypes(compilationUnit, declaringType) && left.resolveTypes(compilationUnit, declaringType)
                && right.resolveTypes(compilationUnit, declaringType);
    }
    ...
    public boolean checkSemantics(CompilationUnit compilationUnit, IType declaringType, IMethod declaringMethod,
        ...
        ITypeCast[] result = TypeCast.findBestCast(rightTypeCastCandidates.toArray(new ITypeCast[rightTypeCastCandidates
                .size()]));
        ...
        if(result.length == 1) {
            ...
            setError(getPosition(),
                    "The " + operatorToken.getName() + " is ambiguous for the type(s) " + leftType.getSignature() + ", "
                            + rightType.getSignature());
            return false;
        }
        ...
        if(rightTypeCastCandidates.size() == 0) {
            setError(getPosition(), "The " + operatorToken.getName() + " is undefined for the argument type(s) "
                    + leftType.getSignature() + ", " + rightType.getSignature());
            return false;
        }
    ...
    public void getCode(CodeGenerator codeGenerator) {
        ...
        if(left.getVariableType().isEnum()) {
            ...
            boolean isBoolean = !leftType.isArray() && leftType.getType() == booleanType && !rightType.isArray()
                    && rightType.getType() == booleanType;
