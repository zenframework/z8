...
import_element
	: IMPORT qualified_name SEMICOLON		{ parser.createImportElement($1); }
	;
	
attribute_lbracket
	: LBRACKET
		{
			parser.startAttribute($1);
		}
...
qualified_name
	: IDENTIFIER
		{
			parser.onQualifiedName(true, $1);
		}
	| qualified_name DOT IDENTIFIER
		{
			parser.onQualifiedName(false, $3);
		}
	| qualified_name DOT
		{
			error($2);
...
class_member
	: class_member2						{ parser.addClassMember(); }	
	;
...
simple_record
	...
	| record_name CONSTANT
		{
			error("'=' expected after this token");
			parser.setRecordValue($2);
		}
	| record_name record_assign CONSTANT
		{
			parser.setRecordValue($3);
		}
	;
...
records_list
	: record						{ parser.addRecord(); }
	| records_list record			{ parser.addRecord(); }
	;
			
type	
	: qualified_name
		{
			parser.onTypeHelper(false);
		}
	| simple_array_access
...
simple_declarator
	: type							{ parser.onSimpleDeclarator(false); }
	| type type						{ parser.onSimpleDeclarator(true); }
	;
	
initialization_left
	: simple_declarator ASSIGN		{ parser.setInitOperator($2); }
	| simple_declarator ADD_ASSIGN	{ parser.setInitOperator($2); }
...
method_header
	...
	| OPERATOR simple_declarator parameters					{ parser.createOperator($1); }
	| simple_declarator OPERATOR NOT parameters				{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR MUL parameters				{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR DIV parameters				{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR MOD parameters				{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR ADD parameters			{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR SUB parameters			{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR EQU parameters				{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR NOT_EQU parameters			{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR LESS parameters			{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR MORE parameters			{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR LESS_EQU parameters		{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR MORE_EQU parameters		{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR AND parameters				{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR OR parameters				{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR ASSIGN parameters			{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR ADD_ASSIGN parameters		{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR SUB_ASSIGN parameters		{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR MUL_ASSIGN parameters		{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR DIV_ASSIGN parameters		{ parser.createOperator($2, $3); }
	| simple_declarator OPERATOR MOD_ASSIGN parameters		{ parser.createOperator($2, $3); }
	| parameters
		{
			parser.removeMethod();
		}
...
method
	...
	| method_header
		{
			if(parser.peek() instanceof ILanguageElement )
			{
				ILanguageElement element = (ILanguageElement)parser.peek();
				error(element.getPosition(), "MethodBody is missing");
			}
		}
	| compound
		{
			parser.removeMethodBody();
			error("MethodHeader is missing");
		}
	;

parameter
	: simple_declarator			{ parser.createParameter(); }
...
array_initializer_lcbrace
	: LCBRACE
		{ 
			parser.startArrayInitializer($1);
		}
	;
...
array_initializer
	...
	| array_initializer_left RCBRACE
		{ 
			parser.finishArrayInitializer($2);
		}
	;
...
statement
	: try_catch_finally
	| throw
	| expression_statement		{ parser.onStatement(); }
	| compound
	| selection
	| iteration
	| jump
...
finally_keyword
	: FINALLY
		{
			parser.startFinallyStatement($1);
		}
	;
...
do_keyword
	: DO
		{ 
			parser.startDoWhileStatement($1);
		}
...
return_keyword
	: RETURN			{ parser.startJumpStatement($1); }
	;
	
break
	: BREAK				{ parser.startJumpStatement($1); }
	;

continue
	: CONTINUE			{ parser.startJumpStatement($1); }
	;
...
additive
	...
	| additive ADD_ASSIGN multiplicative			{ parser.onOperator($2); }
	| additive SUB multiplicative					{ parser.onOperator($2); }
	| additive SUB_ASSIGN multiplicative			{ parser.onOperator($2); }
...
multiplicative
	...
	| multiplicative MUL_ASSIGN unary				{ parser.onOperator($2); }
	| multiplicative DIV unary						{ parser.onOperator($2); }
	| multiplicative DIV_ASSIGN unary				{ parser.onOperator($2); }
	| multiplicative MOD unary						{ parser.onOperator($2); }
	| multiplicative MOD_ASSIGN unary				{ parser.onOperator($2); }
...
braced_expression_left
	: LBRACE expression
		{
			parser.startBracedExpression($1);
		}
	;
...
braced_expression
	...
	| braced_expression_left RBRACE
		{
			parser.finishBracedExpression($2);
		}
	;
...
unary
	...
	| member_access
	| ADD						{ error($1); parser.onEmptyUnary($1); }
	| SUB						{ error($1); parser.onEmptyUnary($1); }
	| NOT						{ error($1); parser.onEmptyUnary($1); }
	;

new_keyword
	: NEW
		{
			parser.startNewExpression($1);
		}
...
this_access
	...
	| THIS DOT								{ parser.onThis($1, false); error($2); }
	| THIS DOT postfix_expression			{ parser.onThis($1, true); }
	| SUPER									{ parser.onSuper($1, false); error($1); }
	| SUPER DOT								{ parser.onSuper($1, false); error($2); }
	| SUPER DOT postfix_expression			{ parser.onSuper($1, true); }
	| container
	| container DOT postfix_expression		{ parser.addContainer(); }
...
postfix_expression
	: postfix_expression1
	| braced_expression
	| braced_expression DOT postfix_expression1 { parser.onPostfix(); }
	;

postfix_expression1
	: qualified_name
	| postfix
	| postfix DOT postfix_expression1		{ parser.onPostfix(); }
...
array_access
	: simple_array_access
		{
			parser.onArrayAccess(true);
		}
	| method_call indices
		{
			parser.onArrayAccess(false);
		}
	;
