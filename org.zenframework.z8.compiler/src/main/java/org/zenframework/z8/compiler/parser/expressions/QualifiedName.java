    ...
    public boolean checkSemantics(CompilationUnit compilationUnit, IType declaringType, IMethod declaringMethod,
            ...
            if(variable == null) {
                ...
                if(variable instanceof IMember) {
                    ...
                    if(member.getDeclaringType() != declaringType && member.isPrivate()) {
                        setError(nameToken.getPosition(), "The member " + member.getDeclaringType().getUserName() + '.'
                                + name + " is not visible");
                    }
                }
        ...
        for(; i < tokens.size(); i++) {
            ...
            if(variableType.isStatic() && !member.isStatic()) {
                setFatalError(nameToken.getPosition(), "Cannot make a static reference to the non-static member " + name
                        + " from " + staticType.getUserName());
                return false;
            }

            if(!variableType.isStatic() && member.isStatic()) {
                setWarning(nameToken.getPosition(), "The static member " + variableType.getSignature() + '.' + name
            ...
            if(memberDeclaringType != declaringType) {
                if(declaringType == null) {
                    if(!member.isPublic()) {
                        setError(nameToken.getPosition(), "The member " + memberDeclaringType.getUserName() + '.' + name
                                + " is not visible");
                    }
                }
                else if(member.isPrivate() && !memberDeclaringType.hasPrivateAccess(declaringType) || member.isProtected()
                        && !memberDeclaringType.hasProtectedAccess(declaringType)) {
                    setError(nameToken.getPosition(), "The member " + memberDeclaringType.getUserName() + '.' + name
                            + " is not visible");
                }
            }
            ...
            compilationUnit.addHyperlink(nameToken.getPosition(), member.getDeclaringType().getCompilationUnit(),
                    member.getPosition());
            compilationUnit.addContentProposal(nameToken.getPosition(), variableType);
        }
