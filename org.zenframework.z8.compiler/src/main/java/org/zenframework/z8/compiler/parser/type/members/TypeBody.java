...
public class TypeBody extends LanguageElement {
    ...
    public boolean checkSemantics(CompilationUnit compilationUnit, IType declaringType, IMethod declaringMethod,
        ...
        for(ILanguageElement member : members) {
            ...
            if(member instanceof Operator) {
                ...
                if(operator.isKindOf(IToken.ASSIGN) && parameters.length == 1
                        && parameters[0].compare(new VariableType(compilationUnit, declaringType))) {
        ...
        if(!declaringType.isNative() && declaringType.extendsPrimary() && assignOperator == null) {
            ...
            setError(getDeclaringType().getPosition(), "The type " + name
                    + " extends a primary type and must implement operator=(" + name + ")");
        }
    ...
    private void getPrimaryTypeConstructor(CodeGenerator codeGenerator) {
        ...
        codeGenerator.indent();
        codeGenerator.append("public " + getDeclaringType().getJavaName() + '(' + getDeclaringType().getJavaName()
                + " value" + ')');
        codeGenerator.breakLine();
        codeGenerator.indent();
        codeGenerator.append("{");
    ...
    private void getReferences(IType type, List<IInitializer> references) {
        ...
        for(IInitializer initializer : initializers) {
            ...
            if(operator == null || operator.getId() != IToken.ASSIGN || !rightElement.isQualifiedName()
                    && !rightElement.isOperatorNew()) {
                continue;
            }
            ...
            for(int index = references.size() - 1; index >= 0; index--) {
                ...
                if(initializer instanceof IType && !(reference instanceof IType) || reference instanceof IType
                        && !(initializer instanceof IType)) {
                    continue;
                }
    ...
    private void arrangeReferences(List<IInitializer> initializers) {
        ...
        while(true) {
            ...
            for(int i = index + 1; i < initializers.size(); i++) {
                ...
                if(leftName.equals(rightName) || leftName.startsWith(rightName + '.')
