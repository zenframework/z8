...
public class Parser {
        ...
        if(expression instanceof UnaryExpression) {
            ...
            if(unary.getOperatorToken().getId() == IToken.MINUS && unary.getExpression() instanceof Constant) {
                ...
                if(constantToken instanceof IntegerToken) {
                    ...
                    IntegerToken integerToken = new IntegerToken(-value, unary.getOperatorToken().getPosition()
                            .union(constantToken.getPosition()));
                ...
                if(constantToken instanceof DecimalToken) {
                    ...
                    DecimalToken integerToken = new DecimalToken(value.negate(), unary.getOperatorToken().getPosition()
                            .union(constantToken.getPosition()));
                    push(new Attribute(leftBracket, name, integerToken, rightBracket));
                    return;
                }
    ...
    class Modifiers {
        ...
        public IToken getFirstToken() {
            ...
            return LanguageElement.getFirstToken(
                    attribute,
                    LanguageElement.getFirstToken(virtualToken,
                            LanguageElement.getFirstToken(accessToken, LanguageElement.getFirstToken(autoToken, staticToken))));
        }
        ...
        boolean checkAttribute(IAttribute attribute) {
            ...
            if(attributes.get(name) != null) {
                getCompilationUnit().error(attribute.getNameToken().getPosition(),
                        "The attribute " + name + " is declared more then once");
                return false;
            }
    ...
    void onMissingSemicolon() {
        ...
        IToken token = (statement instanceof VariableDeclarator) ? ((VariableDeclarator)statement).getFirstToken()
                : ((ILanguageElement)statement).getFirstToken();
            ...
            if(variableType != null) {
                ...
                if(initializer != null) {
                    if(initializer instanceof Type) {
                        ...
                        ILanguageElement nestedType = new MemberNestedType(new QualifiedName(nameToken),
                                type.getClassToken(), type.getBody());
                    ...
                    ILanguageElement memberInitializer = new MemberInit(new QualifiedName(nameToken), operatorToken,
                            initializer);
    ...
    void createMethod(IToken nameToken) {
        ...
        VariableTypeHelper variableTypeHelper = (VariableTypeHelper)pop();
        VariableType variableType = variableTypeHelper.getVariableType();
        push(new Method(variableType, nameToken, parameters.toArray(new Variable[parameters.size()]),
                parameters.leftBrace, parameters.rightBrace));
    }

    void createPriorityOperator(IToken operatorKeyword) {
        Parameters parameters = (Parameters)pop();
        push(new PriorityOperator((KeywordToken)operatorKeyword, parameters.toArray(new Variable[parameters.size()]),
                parameters.leftBrace, parameters.rightBrace));
    }

    void createCastOperator(IToken operatorKeyword, IToken typeNameToken) {
        Parameters parameters = (Parameters)pop();
        push(new CastOperator((KeywordToken)operatorKeyword, new VariableType(typeNameToken),
                parameters.toArray(new Variable[parameters.size()]), parameters.leftBrace, parameters.rightBrace));
    }
    ...
    void createOperator(IToken operatorKeyword, IToken operatorToken) {
        ...
        VariableTypeHelper variableTypeHelper = (VariableTypeHelper)pop();
        VariableType variableType = variableTypeHelper.getVariableType();
        push(new Operator(variableType, (KeywordToken)operatorKeyword, (OperatorToken)operatorToken,
                parameters.toArray(new Variable[parameters.size()]), parameters.leftBrace, parameters.rightBrace));
    }
    ...
    void onMethodCall(boolean hasPrefix) {
        ...
        ILanguageElement methodCall = new MethodCall(prefix, methodNameToken,
                expressions.toArray(new ILanguageElement[expressions.size()]), expressions.leftBrace,
                expressions.rightBrace);
