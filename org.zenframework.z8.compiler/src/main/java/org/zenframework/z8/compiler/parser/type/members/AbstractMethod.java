...
public abstract class AbstractMethod extends LanguageElement implements IMethod {
    ...
    public IToken getFirstToken() {
        return getFirstToken(super.getFirstToken(),
                getFirstToken(accessToken, getFirstToken(virtualToken, getFirstToken(staticToken, variableType.getFirstToken()))));
    }
    ...
    public boolean resolveStructure(CompilationUnit compilationUnit, IType declaringType) {
        ...
        if(method != null) {
            if(method.getDeclaringType() == declaringType) {
                setFatalError(method.getPosition(),
                        "Duplicate method " + getSignature() + " in type " + declaringType.getUserName());
                setFatalError(getPosition(),
                        "Duplicate method " + getSignature() + " in type " + declaringType.getUserName());
                return false;
            }

            if(!variableType.compare(method.getVariableType())) {
                setFatalError(getPosition(), "The return type is incompatible with "
                        + method.getDeclaringType().getUserName() + "." + method.getSignature());
                return false;
            }

            if(!method.isStatic() && isStatic()) {
                if(method.isVirtual()) {
                    setError(getPosition(), "The method " + method.getVariableType().getSignature() + " " + getSignature()
                            + " in type " + method.getDeclaringType().getUserName()
                            + " is virtual and cannot be overriden with a static method");
                }
                else {
                    setError(getPosition(), "The method " + method.getVariableType().getSignature() + " " + getSignature()
                            + " in type " + method.getDeclaringType().getUserName()
                            + " is not static and cannot be overriden with a static method");
                }
            }
            else if(method.isStatic() && !isStatic()) {
                setError(getPosition(), "The method " + method.getVariableType().getSignature() + " " + getSignature()
                        + " in type " + method.getDeclaringType().getUserName()
                        + " is static and cannot be overriden with a non-static method");
            }
            else if(!method.isStatic() && !isStatic()) {
                ...
                if(!method.isVirtual()) {
                    setError(getPosition(), "The method " + method.getVariableType().getSignature() + " " + getSignature()
                            + " in type " + method.getDeclaringType().getUserName()
                            + " is not virtual and cannot be overriden");
                }
            }
        }
        else {
    ...
    public boolean checkSemantics(CompilationUnit compilationUnit, IType declaringType, IMethod declaringMethod,
        ...
        assert (body instanceof IStatement);
