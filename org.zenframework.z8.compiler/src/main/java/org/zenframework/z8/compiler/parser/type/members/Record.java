...
public class Record extends LanguageElement implements IMember {
    ...
    public boolean resolveStructure(CompilationUnit compilationUnit, IType declaringType) {
        ...
        if(member != null) {
            if(member.getDeclaringType() == declaringType) {
                ...
                setError(getPosition(), getName() + ": redefinition of " + member.getDeclaringType().getUserName() + "."
                        + getName());
            }
            return false;
        }
        ...
        if(constant == null) {
            setFatalError(nameToken.getPosition(), "The member " + nameToken.getRawText()
                    + " must be initialized with guid constant");
            return false;
        }

        if(!(constant.getToken() instanceof GuidToken)) {
            setFatalError(constant.getPosition(), "The type " + constant.getToken().getTypeName()
                    + " can not be used here; use guid");
            return false;
        }
    ...
    public boolean checkSemantics(CompilationUnit compilationUnit, IType declaringType, IMethod declaringMethod,
            ...
            for(IMember member : members) {
                if(member instanceof Record) {
                    ...
                    if(element != this && element.getGuidValue().format().equals(getGuidValue().format())) {
                        setError(constant.getPosition(), declaringType.getUserName() + "." + getName()
                                + ": id conflicts with " + type.getUserName() + "." + member.getName());
                        return false;
                    }
                }
            }
        ...
        for(IAttribute attribute : attributes) {
            ...
            if(member == null) {
                setError(attribute.getNameToken().getPosition(), attribute.getName()
                        + " cannot be resolved to a type member name");
                continue;
            }
            ...
            */
            ...
            if(sqlTypeName == null) {
                setError(attribute.getPosition(), "The value of type " + variableType.getSignature()
                        + " cannot be converted to " + member.getVariableType().getSignature() + ", ("
                        + member.getDeclaringType().getUserName() + "." + member.getName() + ")");
                continue;
            }
    ...
    public void getCode(CodeGenerator codeGenerator) {
        ...
        assert (guidType != null);

        codeGenerator.getCompilationUnit().importType(guidType);
        codeGenerator.append("public static " + guidType.getJavaName() + " " + nameToken.getRawText() + " = new "
                + guidType.getJavaName() + "(" + getGuidValue().format(true) + ");");
