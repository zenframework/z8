	...
	{
		m_parser.getCompilationUnit().error(position, message);
	}
	...
	void error(IToken token, String message)
	{
		if(token != null && token.getPosition() != null) // empty or fully commented file
		{
			...
			m_parser.getCompilationUnit().error(token.getPosition(), "Syntax error on token '" + getLexer().getString(token.getPosition()) + message);
		}
		else if(token != null && token.getId() == 0)
		{
			...
			{
				m_parser.getCompilationUnit().error(token.getPosition(), "Syntax error on token '" + getLexer().getString(token.getPosition()) + "', unexpected end of file");
			}
		}
	}
