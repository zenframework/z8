...
Z8.data.Store = Ext.extend(Ext.data.JsonStore,
{
	...
	onException: function(dataProxy, exceptionType, action, request, response, errors)
	{
		...
		if(response.raw != null) // server message
		{
		...
		else if(response.statusText != null) // communication failure
		{
			info = { messages: [response.statusText] };
		}
		else if(Ext.isDefined(errors) && errors != null) // client side exception; exceptionType == 'response'
		{
	...
	isRecordLocked: function(record)
	{
		...
		locked = this.lockProperty != null && record.data[this.lockProperty] === true;
