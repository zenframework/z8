...
Ext.ux.grid.GridFilters = Ext.extend(Ext.util.Observable, {
    ...
    menuFilterCancelText : 'Сбросить фильтры',
    /**
    ...
    /** @private */
    init : function (grid) {
        if (grid instanceof Ext.grid.GridPanel || grid instanceof Z8.tree.TreePanel) {
            this.grid = grid;
        ...
        } else if (grid instanceof Z8.PagingToolbar) {
            this.toolbar = grid;
        }
    },
    ...
    createMenu : function () {
        ...
        if (this.showMenu && hmenu) {
            ...
            this.cancelMenu = hmenu.add({
                itemId: 'cancelFilters',
                text: this.menuFilterCancelText
            });
    ...
    getMenuFilter : function () {
        ...
        return this.filters.get(
            view.getColumnModel().config[view.hdCtxIndex].dataIndex
        );
    },
...
*/
    ...
    onFilterCancel : function (item, value) {
        this.clearFilters();
    },
    ...
    updateColumnHeadings : function () {
        var view = this.grid.getView();
        var colModel = view.getColumnModel();

        if (view.mainHd) {
            for (var i = 0; i < colModel.config.length; i++) {
                var filter = this.getFilter(colModel.config[i].dataIndex);
