...
public class ResultSetMetaData implements IResultSetMetaData {
    private IValue[] m_columns = null;

    public ResultSetMetaData(IValue[] columns) {
        m_columns = columns;
    }
    ...
    public String getColumnLabel(int index) throws OdaException {
        IValue value = m_columns[index - 1];
    ...
    public int getColumnType(int index) throws OdaException {
        if(m_columns[index - 1].type() == FieldType.Guid || m_columns[index - 1].type() == FieldType.String
                || m_columns[index - 1].type() == FieldType.Boolean)
            return 1;
