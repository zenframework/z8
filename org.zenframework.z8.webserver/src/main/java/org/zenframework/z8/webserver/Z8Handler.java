...
public class Z8Handler extends AbstractHandler {
	...
	private static final Collection<UrlPattern> urlPatterns = new LinkedList<UrlPattern>(
			Arrays.asList(
					...
					new UrlPattern("/token_auth"),
					new UrlPattern("*.json"),
					new UrlPattern("/storage/*"),
					new UrlPattern("/files/*"),
					new UrlPattern("/reports/*"),
					new UrlPattern("/redirect"),
					new UrlPattern("/accept")));
	...
	public Z8Handler(ServletContext context) {
		...
		try {
			...
			requestServlet.init(new SimpleConfig("Z8 Servlet", context, ServerConfig.webServerServletParams()));
