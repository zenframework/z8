...
import org.zenframework.z8.server.apidocs.dto.EntityAction;
	...
	protected List<IActionRequest> getActionsDescription(Collection<OBJECT.CLASS<? extends OBJECT>> entities) {
		...
		for(OBJECT.CLASS<? extends OBJECT> entity : entities) {
			if(entity.newInstance() instanceof Query && entity.getAttribute("generatable") != null) {
				...
				IActionRequest[] availableActions = new IActionRequest[] { new CreateAction(), new CopyAction(), new ReadAction(), new UpdateAction(), new DestroyAction(), new CommandAction(), new ExportAction(), new AttachAction(), new DetachAction() };
