...
public class SetDate extends SqlToken {
    ...
    public String format(DatabaseVendor vendor, FormatOptions options, boolean logicalContext) {
        switch(vendor) {
        case SqlServer: {
            String convert = "convert(varchar, " + year.format(vendor, options) + ") + '-' + " + "convert(varchar, "
                    + month.format(vendor, options) + ") + '-' + " + "convert(varchar, " + day.format(vendor, options) + ")";

            if(hour != null) {
                convert += 'T' + "convert(varchar, " + hour.format(vendor, options) + ") + ':' + " + "convert(varchar, "
                        + minute.format(vendor, options) + ") + ':' + " + "convert(varchar, "
                        + second.format(vendor, options) + ")";
            }

            return "cast(" + convert + " as " + (hour == null ? "Date" : "Datetime") + ")";

            /*			
            			String result = hour != null ? "DateTimeFromParts" : "DateFromParts";
            			
            			result += "(" + year.format(vendor, options) + ", " + month.format(vendor, options) + ", " + day.format(vendor, options);
            			
            			if(hour != null)
            			{
            				result += "(" + hour.format(vendor, options) + ", " + minute.format(vendor, options) + ", " + second.format(vendor, options) + ", 0";
            			}
    ...
    public String formula() {
        return " new Date(" + year.formula() + ", " + month.formula() + "-1, " + day.formula() + ")";
    }
}
