...
public class RPad extends StringFunction {
    ...
    public RPad(SqlToken p1, SqlToken p2, SqlToken p3) {
        param1 = p1;
        param2 = p2;
        param3 = p3;
    }
    ...
    public String format(DatabaseVendor vendor, FormatOptions options, boolean logicalContext) {
        ...
        switch(vendor) {
        case Oracle:
            return "RPAD(" + param1.format(vendor, options) + "," + param2.format(vendor, options) + ","
                    + param3.format(vendor, options) + ")";
        case SqlServer:
            return "LEFT(" + param1.format(vendor, options) + "+REPLICATE(" + param3.format(vendor, options) + ","
                    + param2.format(vendor, options) + ")" + "," + param2.format(vendor, options) + ")";
        default:
