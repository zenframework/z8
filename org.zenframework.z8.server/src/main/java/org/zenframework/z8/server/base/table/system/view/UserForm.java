	...
	public static class UniqueUserRoles extends UserRoles
	{
		...
		public static class UniqueRoles extends Roles
		{
			...
			public void z8_beforeRead() {
				super.z8_beforeRead();
				UserRoles.CLASS<? extends UserRoles> rolesUser = new UserRoles.CLASS<UserRoles>(this);
				rolesUser.get().role.get().aggregation = Aggregation.Array;
				if(rolesUser.get().z8_aggregate(new RCollection(new Object[]{ rolesUser.get().role }), rolesUser.get().user.get().sql_guid().operatorEqu(((UserForm.CLASS<UserForm>)getContainer().getContainer().getCLASS()).get().users.get().recordId.get().z8_get().sql_guid())).get())
					z8_addWhere(Sql.z8_inVector(recordId.get().sql_guid(), rolesUser.get().role.get().z8_array()).operatorNot());
			}
		};
		...
		public void z8_beforeCreate(guid recordId) {
			...
			UserRoles.CLASS<? extends UserRoles> rolesUser = new UserRoles.CLASS<UserRoles>(this);
			if(rolesUser.get().z8_readFirst(new RCollection(new Object[]{ rolesUser.get().roles.get().name }), rolesUser.get().user.get().sql_guid().operatorEqu(user.get().z8_get().sql_guid()).operatorAnd(rolesUser.get().role.get().sql_guid().operatorEqu(role.get().z8_get().sql_guid()))).get())
				throw new RuntimeException(Resources.format("Exception.duplicateRole", rolesUser.get().roles.get().name.get()));
		}
	};
}
