	...
	public String format(DatabaseVendor vendor, FormatOptions options, boolean logicalContext) {
		...
		case Sub: {
			...
			if(this.left.isDate() && this.right.isDate()) {
				switch(vendor) {
				case Oracle:
					return "((EXTRACT(DAY FROM (" + left + "-" + right + ") DAY TO SECOND)*" + datespan.TicksPerDay + ")+(EXTRACT(HOUR FROM (" + left + "-" + right + ") DAY TO SECOND)*" + datespan.TicksPerHour + ")+(EXTRACT(MINUTE FROM (" + left + "-" + right
							+ ") DAY TO SECOND)*" + datespan.TicksPerMinute + ")+(EXTRACT(SECOND FROM (" + left + "-" + right + ") DAY TO SECOND)*" + datespan.TicksPerSecond + "))";
				case SqlServer:
					return "((convert(numeric(19,0), DATEDIFF(mi, " + right + ", " + left + "))*" + datespan.TicksPerMinute + ")+DatePart(ss, " + left + "-" + right + ")*" + datespan.TicksPerSecond + "+DatePart(ms, " + left + "-" + right + "))";
				default:
